...
- **Connection Hoarding Guards Across 15 Heavy Processing Handlers**: Enforced strict connection drop-before-await pattern across RemBG, DeOldify, Real-ESRGAN, Vosk STT, TTS, Vocal Separation, Denoise, Package Converter, Magic Studio, Spotify, SoundCloud, MusicSet, Surge DL, and YouTube handlers, guaranteeing checked-out pool connections are never held across slow Telegram file transfers or CPU Broker wait queues.
