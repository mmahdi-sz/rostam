...
fn subtitle_lang_of(fname: &str) -> Option<String> {
...
pub async fn hardsub_subtitles(
    ...
    if srts.is_empty() {
        crate::youtube::trace::log_trace(trace_id, "hardsub_skipped", "no srt found");
        return Ok(video_path.to_string());
    }
    ...
    if let Err(e) = tokio::fs::copy(primary_srt, &hardsub_srt).await {
        crate::youtube::trace::log_trace(trace_id, "hardsub_copy_srt_failed", &e.to_string());
