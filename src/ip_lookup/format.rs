...
pub fn cidr_note(prefix: u8, is_v4: bool) -> String {
    ...
    let hosts = if is_v4 && host_bits <= 32 {
        tf(
            "ip_lookup.note.cidr_hosts",
            &[("count", &(1u64 << host_bits).to_string())],
        )
    } else {
        String::new()
    };
...
pub fn format_report(report: &IpReport, matches: &ListMatches) -> String {
    ...
    if let Some(s) = ownership_section(report) {
        sections.push(s);
    }
    if let Some(s) = location_section(report) {
        sections.push(s);
    }
    if let Some(s) = network_section(report) {
        sections.push(s);
    }
    if let Some(s) = risk_section(report, matches) {
        sections.push(s);
    }
    if let Some(s) = abuse_contact_section(report) {
        sections.push(s);
    }
...
fn ownership_section(report: &IpReport) -> Option<String> {
    ...
    if let Some(org) = report
        .rdap_org
        .as_ref()
        .or(report.ipinfo_org.as_ref())
        .or(report.geo_org.as_ref())
    {
...
fn location_section(report: &IpReport) -> Option<String> {
    ...
    let code = report
        .geo_country_code
        .as_ref()
        .or(report.bgp_country.as_ref());
    match (country, code) {
        (Some(c), Some(code)) if c != code => push_line(
            &mut lines,
            "ip_lookup.label.country",
            &format!("{c} ({code})"),
        ),
    ...
    let latlon = report
        .geo_lat
        .zip(report.geo_lon)
        .or_else(|| report.ipinfo_lat.zip(report.ipinfo_lon));
    ...
    if let Some(tz) = report
        .geo_timezone
        .as_ref()
        .or(report.ipinfo_timezone.as_ref())
    {
...
fn network_section(report: &IpReport) -> Option<String> {
    ...
    let asn = report.bgp_asn.map(|n| n.to_string()).or_else(|| {
        report
            ...
            .map(|s| s.trim_start_matches("AS").to_string())
    });
    if let Some(asn) = asn {
...
fn risk_section(report: &IpReport, matches: &ListMatches) -> Option<String> {
    ...
    push_flag(
        &mut lines,
        "ip_lookup.flag.proxy",
        report.geo_proxy.unwrap_or(false),
    );
    push_flag(
        &mut lines,
        "ip_lookup.flag.hosting",
        report.geo_hosting.unwrap_or(false),
    );
    push_flag(
        &mut lines,
        "ip_lookup.flag.mobile",
        report.geo_mobile.unwrap_or(false),
    );
    push_flag(
        &mut lines,
        "ip_lookup.flag.vpn",
        report.ipinfo_vpn.unwrap_or(false),
    );
    push_flag(
        &mut lines,
        "ip_lookup.flag.tor",
        matches.is_tor || report.ipinfo_tor.unwrap_or(false),
    );
    push_flag(
        &mut lines,
        "ip_lookup.flag.relay",
        report.ipinfo_relay.unwrap_or(false),
    );
    ...
    if let Some(provider) = matches.cloud_provider {
        match &matches.cloud_region {
            Some(region) => push_line(
                &mut lines,
                "ip_lookup.label.cloud_provider",
