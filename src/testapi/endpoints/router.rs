...
use frankenstein::{
    client_reqwest::Bot,
    types::{CallbackQuery, User, Message, Chat, ChatType, MaybeInaccessibleMessage},
...
pub async fn test_callback(Json(payload): Json<Value>) -> axum::response::Response {
    ...
    let cb_data = match payload.get("callback_data") {
        ...
        Some(_) => return (axum::http::StatusCode::BAD_REQUEST, "callback_data must be a string").into_response(),
        None => return (axum::http::StatusCode::BAD_REQUEST, "missing callback_data").into_response(),
    };
    
    let user_id = match payload.get("user_id") {
        Some(v) if v.is_i64() => v.as_i64().unwrap(),
        Some(_) => return (axum::http::StatusCode::BAD_REQUEST, "user_id must be an integer").into_response(),
        None => 12345,
    };
    
    let username = match payload.get("username") {
        Some(v) if v.is_string() => v.as_str().unwrap().to_string(),
        Some(_) => return (axum::http::StatusCode::BAD_REQUEST, "username must be a string").into_response(),
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async {
            let _ = CAPTURED_TRACES.scope(t, async {
    ...
    for payload in &payloads {
        let method = payload.get("_method").and_then(|v| v.as_str()).unwrap_or("");
