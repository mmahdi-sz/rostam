...
pub async fn get_feature_stats(client: &Client, feature: &str) -> Result<FeatureStats, tokio_postgres::Error> {
    let row = client.query_one(
        "SELECT
    ...
    Ok(FeatureStats {
        ok:     Periods { d1: row.get(0),  d3: row.get(1),  d7: row.get(2),  d30: row.get(3) },
        fail:   Periods { d1: row.get(4),  d3: row.get(5),  d7: row.get(6),  d30: row.get(7) },
        amount: Periods { d1: row.get(8),  d3: row.get(9),  d7: row.get(10), d30: row.get(11) },
    })
}
...
pub async fn get_action_breakdown(client: &Client, feature: &str) -> Result<Vec<ActionCount>, tokio_postgres::Error> {
    ...
    Ok(rows.iter().map(|r| ActionCount {
        action: r.get(0),
        status: r.get(1),
        d1:  r.get(2),
        d7:  r.get(3),
...
pub async fn get_recent_errors(client: &Client, limit: i64) -> Result<Vec<ErrorRow>, tokio_postgres::Error> {
    let rows = client.query(
