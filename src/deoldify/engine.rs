...
impl SessionHolder {
    fn new() -> Self {
        Self { session: None, last_used: Instant::now() }
    }
...
fn xyz_to_lab(x: f32, y: f32, z: f32) -> (f32, f32, f32) {
    ...
    let l = if (y / YN) > 0.008856 { 116.0 * fy - 16.0 } else { 903.3 * (y / YN) };
    let a = 500.0 * (fx - fy);
    let b = 200.0 * (fy - fz);
    (l, a, b)
}
...
pub async fn run_deoldify_colorize(
    ...
    let process_res = tokio::task::spawn_blocking(move || -> Result<(), String> {
        ...
        eprintln!("[deoldify trace={trace_id} event=image_loaded] w={orig_w} h={orig_h} load_ms={:.0}",
        ...
        let resized = image::imageops::resize(&rgb8, INPUT_SIZE as u32, INPUT_SIZE as u32, image::imageops::FilterType::CatmullRom);
        ...
        eprintln!("[deoldify trace={trace_id} event=preprocess_done] ms={:.0}",
        ...
        let a_resized = bilinear_resize(a_plane, INPUT_SIZE as u32, INPUT_SIZE as u32, orig_w, orig_h);
        let b_resized = bilinear_resize(b_plane, INPUT_SIZE as u32, INPUT_SIZE as u32, orig_w, orig_h);
        ...
        eprintln!("[deoldify trace={trace_id} event=postprocess_done] ms={:.0}",
        ...
        eprintln!("[deoldify trace={trace_id} event=output_saved] path={}", output_path_buf.display());
