...
use super::{EmojiPack, row_to_pack};
...
pub async fn delete_pack(
    ...
    let was_default = client
        .query_opt(
            "SELECT is_default FROM emoji_packs WHERE id = $1 AND owner_user_id = $2",
            &[&pack_id, &owner],
        )
        .await?
    ...
    let no_packs = client
        .query_one(
            "SELECT COUNT(*) FROM emoji_packs WHERE owner_user_id = $1",
            &[&owner],
        )
        .await
    ...
    if no_packs {
        let _ = client
            .execute("ALTER SEQUENCE emoji_packs_id_seq RESTART WITH 1", &[])
            .await;
        let _ = client
            .execute("ALTER SEQUENCE emoji_items_id_seq RESTART WITH 1", &[])
            .await;
    }
