...
pub fn packs_keyboard(packs: &[EmojiPack]) -> InlineKeyboardMarkup {
    ...
    for pack in packs {
        ...
        rows.push(vec![btn_icon(
            &label,
            &format!("{CB_PACK_OPEN_PREFIX}{}", pack.id),
            icon,
        )]);
    }
...
pub fn pack_choice_keyboard(
    packs: &[EmojiPack],
    page: usize,
    total_pages: usize,
) -> InlineKeyboardMarkup {
    let mut rows: Vec<Vec<InlineKeyboardButton>> = Vec::new();
    rows.push(vec![btn_success(
        &t("emoji.panel.show_pack_links"),
        CB_SHOW_PACK_LINKS,
    )]);
    for pack in packs.iter().rev() {
        ...
        rows.push(vec![btn_icon(
            &pack.name,
            &format!("{CB_PICK_PACK_PREFIX}{}", pack.id),
            icon,
        )]);
    }
    if total_pages > 1 {
...
pub fn cancel_reply_keyboard() -> ReplyKeyboardMarkup {
    ...
    let cancel_btn = KeyboardButton {
        ...
        icon_custom_emoji_id: if icon_id.is_empty() {
            None
        } else {
            Some(icon_id)
        },
...
pub fn pack_delete_confirm_keyboard(pack_id: i32) -> InlineKeyboardMarkup {
    use super::buttons::{btn_icon_danger, btn_icon_success};
    InlineKeyboardMarkup::builder()
        .inline_keyboard(vec![vec![
            btn_icon_success(
                &t("emoji.panel.delete_confirm_yes"),
                &format!("{CB_PACK_DELETE_CONFIRM_PREFIX}{pack_id}"),
                "check",
            ),
            btn_icon_danger(
                &t("emoji.panel.delete_confirm_no"),
                &format!("{CB_PACK_OPEN_PREFIX}{pack_id}"),
                "cross",
            ),
        ]])
        .build()
