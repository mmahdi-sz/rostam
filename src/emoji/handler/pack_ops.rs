...
pub(super) async fn send_cancel_and_panel(api: &Bot, chat_id: i64, trace_id: u64) {
    ...
    let r1 = api
        ...
        .await;
    eprintln!(
        "[emoji trace={trace_id} event=cancel_msg_sent] ok={}",
        r1.is_ok()
    );
    let r2 = api
        .send_message(
        ...
        .await;
    eprintln!(
        "[emoji trace={trace_id} event=panel_sent] ok={}",
        r2.is_ok()
    );
}
...
pub(super) async fn add_emojis_to_pack(
    ...
    for emoji in collected {
        let smart = match emoji_store::allocate_smart_name(client, user_id, &emoji.fallback).await {
            ...
            Err(e) => {
                eprintln!(
                    "[emoji trace={trace_id} event=allocate_smart_name_failed] fallback={:?} err={e:?}",
                    emoji.fallback
                );
                continue;
            }
        ...
        .await
        {
            Ok(item) => {
                eprintln!(
                    "[emoji trace={trace_id} event=add_item_ok] db_id={} smart_name={:?}",
    ...
    eprintln!(
        "[emoji trace={trace_id} event=add_to_pack_done] added={added} total={}",
        collected.len()
    );
    let summary_text = tf(
        "emoji.added_summary",
        &[("count", &added.to_string()), ("pack", &pack_name)],
    );
    let r = api
        .send_message(
            &SendMessageParams::builder()
        ...
        .await;
    eprintln!(
        "[emoji trace={trace_id} event=summary_sent] ok={}",
        r.is_ok()
    );
    let r = api
        .send_message(
...
pub(super) async fn show_packs_menu(
    ...
    eprintln!(
        "[emoji trace={trace_id} event=packs_loaded] count={}",
        packs.len()
    );
    if packs.is_empty() {
        ...
        let r = api
            ...
            .await;
        eprintln!(
            "[emoji trace={trace_id} event=no_packs_sent] ok={}",
            r.is_ok()
        );
        return;
    }
    edit_panel(
        ...
        chat_id,
        message_id,
        &crate::i18n::t("emoji.packs_title"),
...
pub(super) async fn show_pack_detail(
    ...
    eprintln!(
        "[emoji trace={trace_id} event=show_pack_detail] chat_id={chat_id} pack_id={pack_id}"
    );
    ...
    eprintln!(
        "[emoji trace={trace_id} event=pack_found] name={:?} items={}",
