...
pub(super) async fn handle(
    ...
    if text.starts_with('-') || text.starts_with('+') {
        eprintln!(
            "[emoji_msg trace={trace_id} event=pack_choice_edit_ops] ops={:?}",
        ...
        eprintln!(
            "[emoji_msg trace={trace_id} event=edit_ops_applied] remaining={}",
            collected.len()
        );
        let total_pages = emoji_panel::pending_total_pages(collected.len());
    ...
    if text.starts_with("http") {
        eprintln!(
            "[emoji_msg trace={trace_id} event=pack_choice_url_abort] url_preview={:?}",
    ...
    let pack = match emoji_store::find_pack_by_name(client, user_id, text).await {
        Ok(Some(p)) => {
            eprintln!(
                "[emoji_msg trace={trace_id} event=pack_found] id={} name={:?}",
                p.id, p.name
            );
            p
        }
        Ok(None) => {
            ...
            match emoji_store::create_pack(client, user_id, text).await {
                Ok(p) => {
                    eprintln!(
                        "[emoji_msg trace={trace_id} event=pack_created] id={} name={:?}",
                        p.id, p.name
                    );
                ...
                Err(e) => {
                    eprintln!(
                        "[emoji_msg trace={trace_id} event=pack_create_failed] name={text:?} err={e:?}"
                    );
                    let _ =
                        crate::bot::send_text(api, chat_id, &t("emoji.pack_create_failed")).await;
                    flow_manager.clear(user_id);
                    return true;
                }
            }
        ...
        Err(e) => {
            eprintln!(
                "[emoji_msg trace={trace_id} event=find_pack_failed] name={text:?} err={e:?}"
            );
    ...
    for emoji in &collected {
        let smart = match emoji_store::allocate_smart_name(client, user_id, &emoji.fallback).await {
            ...
            Err(e) => {
                eprintln!(
                    "[emoji_msg trace={trace_id} event=allocate_name_failed] fallback={:?} err={e:?}",
                    emoji.fallback
                );
                continue;
            }
        ...
        .await
        {
            Ok(item) => {
                eprintln!(
                    "[emoji_msg trace={trace_id} event=add_item_ok] db_id={} smart_name={:?}",
                    item.id, item.smart_name
                );
                added += 1;
            }
            Err(e) => {
    ...
    eprintln!(
        "[emoji_msg trace={trace_id} event=pack_choice_done] added={added} total={}",
        collected.len()
    );
    ...
    send_with_ents(
        ...
        Some(ReplyMarkup::ReplyKeyboardRemove(
