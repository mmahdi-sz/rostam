...
pub(super) async fn fetch_pack_emojis(
    api: &Bot,
    pack_name: &str,
    trace_id: u64,
) -> Vec<PendingEmoji> {
    ...
    let set = match api
        .get_sticker_set(&GetStickerSetParams::builder().name(pack_name).build())
        .await
    {
        Ok(r) => r.result,
        Err(e) => {
            eprintln!(
                "[emoji_add trace={trace_id} event=fetch_pack_failed] pack_name={pack_name:?} err={e}"
            );
            return Vec::new();
        }
    };
    let emojis: Vec<PendingEmoji> = set
        .stickers
        .into_iter()
        .filter_map(|s| {
        ...
        .collect();
    eprintln!(
        "[emoji_add trace={trace_id} event=fetch_pack_ok] pack_name={pack_name:?} count={}",
        emojis.len()
    );
    emojis
}
...
pub async fn handle_addemoji_link(
    ...
    if new_emojis.is_empty() {
        ...
        let _ = send_text(
            api,
            chat_id,
            &tf("emoji.pack_link_empty", &[("name", pack_name)]),
        )
        .await;
        return;
    }
    ...
    let r = api
        ...
        .await;
    eprintln!(
        "[emoji_add trace={trace_id} event=pending_sent] ok={}",
        r.is_ok()
    );
    if let Err(e) = r {
        eprintln!("[emoji_add trace={trace_id} event=pending_send_failed] err={e}");
    }
