...
pub(super) fn apply_edit_ops(
    collected: &mut Vec<PendingEmoji>,
    text: &str,
) -> Result<(), &'static str> {
    ...
    if !plus.is_empty() {
        ...
        for idx in plus {
            if idx >= 1 && idx <= snapshot.len() {
                ...
                if !collected
                    .iter()
                    .any(|e| e.custom_emoji_id == candidate.custom_emoji_id)
                {
...
pub(super) fn build_import_report(a: &emoji_import::ImportAnalysis) -> String {
    if a.db_empty {
        format!(
            "{}\n\n{}\n\n{}",
            tf(
                "emoji.import.file_stats",
                &[
                    ("packs", &a.file_packs.to_string()),
                    ("items", &a.file_items.to_string())
                ]
            ),
        ...
        format!(
            ...
            tf(
                "emoji.import.db_stats",
                &[
                    ("packs", &a.db_packs.to_string()),
                    ("items", &a.db_items.to_string()),
                    ("dupes", &a.duplicate_items.to_string())
                ]
            ),
