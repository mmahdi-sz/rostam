...
pub async fn open_emoji_panel(
    ...
    eprintln!(
        "[emoji_open trace={trace_id} event=panel_sent] ok={}",
        r.is_ok()
    );
    if let Err(e) = r {
        eprintln!("[emoji_open trace={trace_id} event=panel_send_failed] err={e}");
    }
}
...
pub async fn handle_emoji_command(
    ...
    let r = api.send_message(&params).await;
    eprintln!(
        "[emoji_cmd trace={trace_id} event=panel_sent] ok={}",
        r.is_ok()
    );
    if let Err(e) = r {
        eprintln!("[emoji_cmd trace={trace_id} event=panel_send_failed] err={e}");
    }
}
...
pub async fn handle_se_command(
    ...
    eprintln!(
        "[emoji_cmd trace={trace_id} event=se_cmd] user_id={user_id} pairs={}",
        pairs.len()
    );
    ...
    for (selector, alias) in &pairs {
        ...
        match emoji_store::set_item_alias(client, user_id, selector, alias_value).await {
            ...
            Err(e) => {
                eprintln!(
                    "[emoji_cmd trace={trace_id} event=se_db_failed] selector={selector:?} err={e:?}"
                );
            }
        }
    }
    ...
    if !not_found.is_empty() {
        let _ = send_text(
            api,
            chat_id,
            &format!("{}: {}", t("emoji.se_not_found"), not_found.join(", ")),
        )
        .await;
    }
