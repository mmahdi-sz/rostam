...
pub(super) async fn send_with_ents(
    api: &Bot,
    chat_id: i64,
    text: String,
    ...
    let params = match (ents.is_empty(), reply_markup) {
        (true, None) => SendMessageParams::builder()
            .chat_id(chat_id)
            .text(text)
            .build(),
        (true, Some(rm)) => SendMessageParams::builder()
            .chat_id(chat_id)
            .text(text)
            .reply_markup(rm)
            .build(),
        (false, None) => SendMessageParams::builder()
            .chat_id(chat_id)
            .text(text)
            .entities(ents)
            .build(),
        (false, Some(rm)) => SendMessageParams::builder()
            .chat_id(chat_id)
            .text(text)
            .entities(ents)
            .reply_markup(rm)
...
pub(super) async fn edit_panel(
    ...
    match api.edit_message_text(&params).await {
        Ok(_) => eprintln!(
            "[emoji trace={trace_id} event=edit_panel_ok] chat_id={chat_id} msg_id={message_id}"
        ),
        Err(e) => eprintln!(
            "[emoji trace={trace_id} event=edit_panel_failed] chat_id={chat_id} msg_id={message_id} err={e}"
        ),
    }
}
