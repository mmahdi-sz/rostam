...
pub(super) async fn send_emoji_list(
    ...
    for pack in packs {
        let items = emoji_store::list_items(client, pack.id)
            .await
            .unwrap_or_default();
    ...
    eprintln!(
        "[emoji trace={trace_id} event=list_loaded] packs={} items={total_items}",
        packs_with_items.len()
    );
...
pub(super) async fn edit_emoji_list_page(
    ...
    match api.edit_message_text(&params).await {
        Ok(_) => eprintln!(
            "[emoji trace={trace_id} event=edit_list_ok] page={page} total_pages={total_pages}"
        ),
