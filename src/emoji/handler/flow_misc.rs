use frankenstein::{
    AsyncTelegramApi, ParseMode, client_reqwest::Bot, methods::SendMessageParams, types::Message,
};
...
pub(super) async fn handle_pack_alias(
    ...
    match emoji_store::set_pack_alias(client, user_id, pack_id, alias).await {
        Ok(_) => eprintln!(
            "[emoji_msg trace={trace_id} event=pack_alias_set] pack_id={pack_id} alias={alias:?}"
        ),
        Err(e) => eprintln!(
            "[emoji_msg trace={trace_id} event=pack_alias_failed] pack_id={pack_id} err={e}"
        ),
    }
    let r = crate::bot::send_text(api, chat_id, &t("emoji.pack_alias_set")).await;
    eprintln!(
        "[emoji_msg trace={trace_id} event=alias_confirm_sent] ok={}",
        r.is_ok()
    );
    flow_manager.clear(user_id);
...
pub(super) async fn handle_test_text(
    ...
    let (rendered, lookups) = if let Some(cache_arc) = cache::global() {
        ...
        eprintln!(
            "[emoji_test trace={trace_id} event=cache_state] source=empty_fallback reason=cache_disabled"
        );
    ...
    match send_result {
        ...
        Err(e) => {
            ...
            match crate::bot::send_text(api, chat_id, &fallback_text).await {
                ...
                Err(fe) => {
                    eprintln!("[emoji_test trace={trace_id} event=fallback_failed] error={fe}",)
                }
            }
        }
    }
